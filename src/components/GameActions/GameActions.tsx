...
const GameActions: React.FC<GameActionsProps> = ({ selectedRoom }) => {
    const { setWaitingForSecondUserToRespond, disabled, setDisabled } = usePresentationLogic();
    ...
    return (
        <>
            <div>
                <h2 style={{ textAlign: 'center' }} data-testid="number-display">{number || ''}</h2>
            </div>
            <div className="game-actions">
                {number ? buttonValues.map(value => (
                    <Button
                        key={value}
