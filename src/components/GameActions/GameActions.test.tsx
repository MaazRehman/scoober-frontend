...
describe('<GameActions />', () => {
    beforeEach(() => {
        ...
        mockUseUserInfo.mockReturnValue({ username: 'TestUser' });
    });
    ...
    it('should render number and enabled buttons based on buttonValues', () => {
        render(
            <GameDataProvider>
                <GameActions selectedRoom="Room Berlin CPU" />
            </GameDataProvider>
        );
        ...
        buttons.forEach((button, index) => {
            ...
            expect(button).toHaveAttribute('data-testid', `button-${buttonValues[index]}`);
        });
    });
    ...
    it('should call sendSelectedNumber event when button is clicked', () => {
        ...
        expect(emit).toHaveBeenCalledWith(events.sendNumber, { number: 5, selectedNumber: 0 });
    });
});
