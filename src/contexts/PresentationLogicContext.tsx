...
const PresentationLogicContext = createContext<PresentationLogicContextType | undefined>(undefined);

interface PresentationLogicProviderProps {
    children: ReactNode;
}

export const PresentationLogicProvider: React.FC<PresentationLogicProviderProps> = ({ children }) => {
    const [waitingForSecondUser, setWaitingForSecondUser] = useState(false);
    const [waitingForSecondUserToRespond, setWaitingForSecondUserToRespond] = useState(false);
    const [disabled, setDisabled] = useState(false);

    return (
        <PresentationLogicContext.Provider value={{
...
export const usePresentationLogic = (): PresentationLogicContextType => {
    ...
    if (!context) {
        throw new Error("usePresentationLogic must be used within a PresentationLogicProvider");
    }
    return context;
};
