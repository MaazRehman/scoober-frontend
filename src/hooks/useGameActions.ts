...
const useGameActions = ({ selectedRoom }: UseGameActionsProps) => {
    ...
    const { setDisabled, setWaitingForSecondUser, setWaitingForSecondUserToRespond } = usePresentationLogic();

    const [number, setNumber] = useState<number>(0);

    const handleRandomNumber = useCallback((data: {
        number: number;
        ...
        if (data.user !== username) {
            ...
            const record: CalculateRecordResult = calculateRecord(numberToUse, data.selectedNumber, data.user);
    ...
    }, [gameData, selectedRoom, setGameData, username, number, setWaitingForSecondUser, setDisabled]);

    const handleActivateYourTurn = useCallback((data: {
        user: string;
        state: string;
    }) => {
        const isMePlaying = data.user === socketId && data.state === gameState.play;
        const isOtherUserWaiting = data.user !== socketId && data.state === gameState.wait;
    ...
    }, [username, setDisabled,setWaitingForSecondUserToRespond, setIsWon,  setIsLost, setNumber]);
