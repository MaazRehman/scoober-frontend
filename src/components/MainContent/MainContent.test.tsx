...
describe('<MainContent />', () => {
  const mockUseGameData = useGameData as jest.MockedFunction<
    typeof useGameData
  >;
  const mockUseGameStatus = useGameStatus as jest.MockedFunction<
    typeof useGameStatus
  >;
  const mockUsePresentationLogic = usePresentationLogic as jest.MockedFunction<
    typeof usePresentationLogic
  >;
  const mockUseUserInfo = useUserInfo as jest.MockedFunction<
    typeof useUserInfo
  >;
  ...
  beforeEach(() => {
    ...
    (GameStep as jest.Mock).mockImplementation(
      ({ step, equation, result, username }) => (
    ...
    (GameActions as jest.Mock).mockImplementation(() => (
      <div data-testid="mock-game-actions">Game Actions</div>
    ));
    (DecisionFrame as jest.Mock).mockImplementation(() => (
      <div data-testid="mock-decision-frame">Decision Frame</div>
    ));
  ...
  it('renders MainContent component with all child components', () => {
    ...
    expect(
      screen.queryByTestId(
        'mock-loader-waiting for second user to play their turn'
      )
    ).toBeNull();
    expect(
      screen.queryByTestId('mock-loader-Waiting for the second user to join')
    ).toBeNull();
